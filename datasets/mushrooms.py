...
class Mushrooms(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        if os.path.exists(self.split_path):
            ...
            trainval = self.read_data(split_file="trainval.txt", img_dir=self.dataset_dir)
            test = self.read_data(split_file="test.txt", img_dir=self.dataset_dir)
    ...
    def read_split(filepath, img_dir):
        def _convert(items):
            ...
            for impath, label, classname in items:
                impath = os.path.join(img_dir, impath)
                print(f"impath:{impath}, img_dir:{img_dir}")
